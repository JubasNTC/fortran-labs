...
program reference_lab_1_2
   ...
   call Read_class_list(input_file, Surnames, Genders, Marks, Aver_Marks)

   call Output_class_list(output_file, Surnames, Genders, Marks, Aver_Marks, &
      "Исходный список:", "rewind")
      
   call Get_list_by_gender(Surnames, Genders, Marks, Boys_Surnames, &
      Boys_Marks, Boys_Aver_Marks, MALE)
   call Get_list_by_gender(Surnames, Genders, Marks, Girls_Surnames, &
      Girls_Marks, Girls_Aver_Marks, FEMALE)

   call Sort_class_list(Boys_Surnames, Boys_Marks, Boys_Aver_Marks)
   call Sort_class_list(Girls_Surnames, Girls_Marks, Girls_Aver_Marks)

   call Output_class_list(output_file, Boys_Surnames, [(MALE, i = 1, Size(Boys_Aver_Marks))], &
      Boys_Marks, Boys_Aver_Marks, "Успеваемость юношей:", "append")
   call Output_class_list(output_file, Girls_Surnames, [(FEMALE, i = 1, Size(Girls_Aver_Marks))], &
      Girls_Marks, Girls_Aver_Marks, "Успеваемость девушек:", "append")
contains
   ! Чтение списка класса: фамилии, инициалы, полы, оценки и средний.
   subroutine Read_class_list(Input_File, Surnames, Genders, Marks, Aver_Marks)
      character(*)         Input_File
      character(kind=CH_)  Surnames(:, :), Genders(:)
      integer              Marks(:, :)
      real(R_)             Aver_Marks(:)
      intent (in)          Input_File
      intent (out)         Surnames, Genders, Marks, Aver_Marks

      integer In, IO, i
      character(:), allocatable  :: format
      
      open (file=Input_File, encoding=E_, newunit=In)
         format = '(' // SURNAME_LEN // 'a1, 1x, a, 1x, ' // &
            MARKS_AMOUNT // 'i1, f5.2)'
         read (In, format, iostat=IO) (Surnames(i, :), Genders(i), Marks(i, :), Aver_Marks(i), &
            i = 1, STUD_AMOUNT)
   ...
   ! Вывод списка класса.
   subroutine Output_class_list(Output_File, Surnames, Genders, Marks, Aver_Marks, List_name, Position)
      character(*)         Output_File, Position, List_name
      character(kind=CH_)  Surnames(:, :), Genders(:)
      integer              Marks(:, :)
      real(R_)             Aver_Marks(:)
      intent (in)          Output_File, Surnames, Genders, Marks, Aver_Marks, List_name, Position
      ...
      open (file=output_file, encoding=E_, position=position, newunit=Out)
         ...
         write (Out, format, iostat=IO) &
            (Surnames(i, :), Genders(i), Marks(i, :), Aver_Marks(i), i = 1, Size(Aver_Marks))
         call Handle_IO_status(IO, "writing " // List_name)
      close (Out)
   end subroutine Output_class_list

   ! Получение списков по полу.
   pure subroutine Get_list_by_gender(Surnames, Genders, Marks, &
         Gender_Surnames, Gender_Marks, Gender_Aver_Marks, Gender)
      character(kind=CH_)  Surnames(:, :), Genders(:)
      integer              Marks(:, :)
      character(kind=CH_)  Gender_Surnames(:, :)
      integer              Gender_Marks(:, :)
      real(R_)             Gender_Aver_Marks(:)
      character(kind=CH_)  Gender
      intent(in)           Surnames, Genders, Marks, Gender
      intent(out)          Gender_Surnames, Gender_Marks, Gender_Aver_Marks
      allocatable          Gender_Surnames, Gender_Marks, Gender_Aver_Marks
      ...
      allocate (Gender_Surnames(Gender_Amount, SURNAME_LEN), &
         Gender_Marks(Gender_Amount, MARKS_AMOUNT))
   ...
   pure subroutine Sort_class_list(Surnames, Marks, Aver_Marks)
      character(kind=CH_)  Surnames(:, :)
      integer              Marks(:, :)
      real(R_)             Aver_Marks(:)
      intent (inout)       Surnames, Marks, Aver_Marks
      ...
      do i = Size(Aver_Marks), 2, -1
         ...
         do j = 1, i-1
            ...
            if (Swap(Aver_Marks, Surnames, j)) &
               call Swap_from_current(Surnames, Marks, Aver_Marks, j)
         end do
      end do
   end subroutine Sort_class_list
      
   ! Проверка того, стоит ли менять местами текущего учащегося со следующим.
   pure logical function Swap(Aver_Marks, Surnames, j)
      character(kind=CH_)  Surnames(:, :)
      real(R_)             Aver_Marks(:)
      integer              j
      intent (in) :: Surnames, Aver_Marks, j
      ...
      if (Aver_Marks(j) < Aver_Marks(j+1)) then
         ...
         if (GT(Surnames(j, :), Surnames(j+1, :))) then
            Swap = .true.
         else if (All(Surnames(j, :) == Surnames(j+1, :))) then
            Swap = .true.
         end if
      end if
   end function Swap
   ...
   pure subroutine Swap_from_current(Surnames, Marks, Aver_Marks, j)
      character(kind=CH_)     Surnames(:, :)
      integer                 Marks(:, :)
      real(R_)                Aver_Marks(:)
      integer, intent(in)  :: j
